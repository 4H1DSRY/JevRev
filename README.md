...
| **JevLoop** | Audit a completed agent round and choose continue, revise, stop, or ask | Roadmap |
| **JevLong / JevKanban** | Watch long sessions for drift, stalls, repeated failures, and budget risk | Roadmap |

JevLoop and JevLong are product direction, not features claimed by this release.
There is no hidden always-on loop or monitoring daemon.
...
`jevrev doctor --format json --check` to inspect configured endpoints.
