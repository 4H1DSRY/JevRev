...
jevrev loop next \
  ...
  --format json > round-work-order.json
```
...
Record benchmark samples from a small JSON file:
...
jevrev evidence metric \
  ...
  --result pass \
  --probe probe-2 \
  --requirement success:throughput
```

The file contains `id`, `criterion_id`, `unit`, `direction`, and raw
`baseline_samples` / `candidate_samples`. JevRev recomputes summary statistics;
it does not infer whether a business threshold passed, so `--result` is
explicit whenever a frozen evidence slot is linked.

Record a diff, screenshot, HTML demo, or other artifact:

```bash
jevrev evidence artifact \
  --evidence evidence.json \
  --candidate ui-probe \
  --id screenshot \
  --file artifacts/home.png \
  --evaluation-id visual-check \
  --evaluator playwright \
  --result pass \
  --summary "Layout and interaction checks passed" \
  --criterion visual \
  --requirement success:visual
```

Artifact paths are workspace-bound and stored by hash. Evaluations are marked
`imported`; JevRev does not pretend that a supplied evaluator result was
generated by JevRev itself.

Ask what is still missing at any point:
